...
// console.log(comments);
    const handleSubmit = async (e) => {
        ...
        if(comment.length  > 200) {
            setCommentError('Only 200 character allowed')
            return;
        }
        try {
            ...
            if(res.ok) {
                ...
                setComments(comments.map((comment) => 
                    comment._id === commentId ? {
                        ...comment,
                        likes: data.likes,
                        numeberOfLikes: data.likes.length,
                    } : comment
                ))
            }
        } catch (error) {
  ...
  return (
    <div className='max-w-2xl mx-auto w-full p-3'>
        {currentUser ?
            ...
            (
                <div className='flex items-center gap-1 my-5 text-gray-500 text-sm'>
                    ...
                    <Link to={'/sign-in'} className='text-sm text-teal-400 hover:underline'>
                        Sign In
                    </Link>
                </div>
            )
        ...
        {
            currentUser && (
                <form className='border border-teal-500 rounded-sm p-3'
